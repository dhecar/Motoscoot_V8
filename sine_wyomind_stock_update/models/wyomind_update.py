...
class WyomindConfig(models.Model):
    _name = 'wyomind.config'
    _description = 'Configuration for Wyomind API stock update'
    _table = 'wyomind_config'

    url = fields.Char(string='Api Url', help="Example --> http://WEBSITE/index.php/api/xmlrpc/", size=60)
    apiuser = fields.Char(string='Api User', size=15)
    apipass = fields.char(string='Api Password', size=15)

WyomindConfig()



class WyomindStockSync(models.TransientModel):
    _name = 'wyomind.sync'
    _description = 'Syncronize Stock'

    @api.multi
    def wyomind_sync(self, values):
        # Wyomind Config

        db_obj = self.pool['base.external.dbsource']

        self.env.cr.execute(""" SELECT SUM(qty) , product_id , location_id FROM stock_quants
        ...
        for r in res:
            ...
            self.env.cr.execute('SELECT magento_id'
        ...
        for move in self.browse(cr, uid, ids, context=context):
            ...
            if move.product_id.magento_bind_ids and move.location_dest_id.id not in (0, 22, 24, 25) and move.picking_id:
                ...
                if move.location_dest_id.id in (12, 15, 19):
                    self.env.cr.execute("""SELECT qty FROM stock_report_prodlots WHERE
...
class InvoiceSaleOrder(models.TransientModel):
...
class stock_change_product_qty(models.osv_memory):
